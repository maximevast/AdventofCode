...
const lines = readRawInput(testInput2);
const myNb = lines.map((line) =>
  line.split("|")[0].split(":")[1].split(" ").filter(Boolean)
);
const valid = lines.map((line) =>
  line.split("|")[1].split(" ").filter(Boolean)
);
